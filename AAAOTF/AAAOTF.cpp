...
AOTF::AOTF() :
   ...
   intensity_(100),
   maxintensity_(1900),
   frequency_(115.32) // Default frequency
...
int AOTF::Initialize()
{
   ...
   if (ret!=DEVICE_OK)
	   return ret;
   SetPropertyLimits(g_Maxint, 0, 3200);//change this to allow going up to full power - to 3300
   ...
   ret = CreateProperty(g_Frequency, "115.32", MM::Float, false, pAct);
   if (ret != DEVICE_OK)
       return ret;
   SetPropertyLimits(g_Frequency, 80.0, 153.0); // Set limits between 80 and 120 MHz 
...
int AOTF::OnFrequency(MM::PropertyBase* pProp, MM::ActionType eAct)
{
    ...
    {
        double tmpFrequency;
        pProp->Get(tmpFrequency);

        // Construct and send the frequency command
        ostringstream command;
        int channel = atoi(activeChannel_.c_str());
        command << "L" << channel << "F" << tmpFrequency;

        int ret = SendSerialCommand(port_.c_str(), command.str().c_str(), "\r");
        if (ret != DEVICE_OK)
            return ret;

        frequency_ = tmpFrequency;
    }
    return DEVICE_OK;
}
